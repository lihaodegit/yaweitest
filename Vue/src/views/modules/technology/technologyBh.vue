    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <!--<el-form-item>-->
        <el-row>
		  ...
		  <el-button style="margin-left:-45px;"  type="primary" @click="confirmBh()" :disabled="dataListSelections.length <= 0">批量驳回</el-button>
        </el-row>
      <!--</el-form-item>-->
    </el-form>
    <br>
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.applystatus == '12'" size="small" type="danger">市科技局复审驳回</el-tag>
          <el-tag v-else-if="scope.row.applystatus == '13'" size="small" type="success">补贴已发放</el-tag>
          <el-tag v-else-if="scope.row.applystatus == '14'" size="small" type="success">已确认</el-tag>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        console.log(params)
        this.$http({
          url: this.$http.adornUrl('/technologyDb/devapply/technologyBhApplyList'),
          method: 'get',
      ...
      detailHandle (id,baseid) {
        this.$router.push({path:'/technologyProgress-detail',name:"technologyProgress-detail",params: {  baseid: baseid,id:id}});
      },
	  // 删除
      confirmBh (id) {
        ...
        this.$confirm(`确定对所选车辆进行${id ? '驳回' : '批量驳回'}操作?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$http({
            url: this.$http.adornUrl('/technologyDb/devapply/batchBhToSeller'),
            method: 'post',
