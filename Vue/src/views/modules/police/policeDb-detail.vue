<template>
	  <div>
	    ...
	    <el-row style="min-height:300px;border-radius: 8px;">
	      ...
	      <el-button v-show="dataForm.applystatus == '1'" type="primary" style="margin-left: 30%;margin-top: 30px;width: 140px;" @click="beforeCommit('3X')">通过</el-button>
	      <el-button v-show="dataForm.applystatus == '1'" type="primary" style="width: 140px;" @click="beforeCommit('4')">驳回</el-button>
	    <el-dialog
	    title=""
	    ...
	    <el-form-item :label="dataForm.dialogTitle">
			...
			<el-form-item label="驳回原因" v-if="dataForm.status == '4'">
				<el-select  v-model="dataForm.policeopinion" placeholder="请选择驳回原因" style="width:100%;">
						<el-option
  ...
  export default {
    ...
    methods: {
			//放大图片
			 ...
			 getBhReason (){
				 this.$http({
				 url: this.$http.adornUrl(`/police/devapply/getBhReasonList`),
				 method: 'get',
          ...
          var opinion = this.dataForm.opinion
					var policeopinion = this.dataForm.policeopinion
          if(opinion == ''){
						...
						this.$http({
							...
							params: this.$http.adornParams(
							{ id:this.dataForm.id,
							  applystatus:this.dataForm.status,
								orgid:this.dataForm.orgid,
								opinion:this.dataForm.opinion,
								policeopinion:policeopinion,
							},
							)
						}).then(({data}) => {
							if (data && data.code === 0) {
