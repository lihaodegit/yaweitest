        ...
        label="车辆基础信息表主键">
			...
			<el-table-column
				...
				<template slot-scope="scope">
					...
					<el-tag v-if="scope.row.applystatus=='1'" size="small" type="success" with="250">市科技局初审通过</el-tag>
...
<script>
	import detail from './policeDb-detail'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/police/devapply/fslist'),
          method: 'get',
      ...
      addOrUpdateHandle (id) {
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdate.init(id)
			...
			//详情
			detail (id,baseid) {
				 this.$router.push({path:'/policeDb-detail',name:"policeDb-detail",params: {  baseid: baseid,id:id}});
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/police/devapply/delete'),
            method: 'post',
			...
			getApplyStatus (row, column, cellValue, index){
				  if(row.applystatus == "1X"){
						return '初审通过'
