  ...
  export default {
    ...
    methods: {
		init(){
			//查询-初审待提交数量
			this.$http({
				url:this.$http.adornUrl('/police/unisubmit/count'),
				method:'post'
			}).then(({data})=>{
				if(data&&data.code===0){
		...
		uniSubmit(){
			this.$http({
				url:this.$http.adornUrl('/police/unisubmit/policeunisubmit'),
				method:'post'
			}).then(({data})=>{
				if(data&&data.code===0){
					this.$message({
		...
		applyList(flag){
			...
			if(flag=='1'){
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'police',flag:'1'}});
			}
			//已提交列表
			else if(flag=='2'){
				if(this.ytjcnt=='0条'){
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'police',flag:'2'}});
			}
		}
    }
  }
