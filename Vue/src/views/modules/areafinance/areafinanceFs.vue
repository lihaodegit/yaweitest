        ...
        label="车辆基础信息表主键">
			...
			<el-table-column
				...
				<template slot-scope="scope">
					<el-tag v-if="scope.row.applystatus=='11'" size="small" type="success">市财政局复审通过</el-tag>
				</template>
			</el-table-column>
    <!--  <el-table-column
        prop="boardtime"
...
<script>
	import detail from './areafinanceFs-detail'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/areafinance/devapply/fslist'),
          method: 'get',
      ...
      addOrUpdateHandle (id) {
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdate.init(id)
			...
			//详情
			detail (id,baseid) {
				 this.$router.push({path:'/areafinanceFs-detail',name:"areafinanceFs-detail",params: {  baseid: baseid,id:id}});
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/areafinance/devapply/delete'),
            method: 'post',
