<template>
  <div class="mod-config">
    ...
    <el-table
      ...
      <el-table-column
		...
		<!--
          ...
          <el-button v-if="scope.row.auditresult !=1" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
          <el-button v-if="scope.row.auditresult !=1" type="text" size="small" @click="updateHandle(scope.row.id)">修改</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <update v-if="updateDetailVisible" ref="Update" @refreshDataList="getDataList"></update>
  </div>
</template>

<script>
  import AddOrUpdate from './devcarqualifapply-add-or-update'
  import ViewDetail from './devcarqualifapply-view'
  import Update from './devcarqualifapply-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/qualifapply/devcarqualifapply/list'),
          method: 'get',
      ...
      addOrUpdateHandle (id) {
		this.$http({
			url: this.$http.adornUrl('/qualifapply/devcarqualifapply/canadd'),
			method: 'post'
		}).then(({data})=>{
			if (data && data.code === 0) {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/qualifapply/devcarqualifapply/delete'),
            method: 'post',
