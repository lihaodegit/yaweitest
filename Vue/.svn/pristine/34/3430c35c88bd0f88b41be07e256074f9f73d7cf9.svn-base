<template>
  <el-dialog
    ...
    <el-form-item label="车企负责人" prop="princ">
      <el-input v-model="dataForm.princ" placeholder="车企负责人"></el-input>
    </el-form-item>
    ...
    <el-form-item label="审核结果:1,同意;0,驳回" prop="auditresult">
  ...
  export default {
    data () {
      return {
        ...
        dataRule: {
          ...
          princ: [
            { required: true, message: '车企负责人不能为空', trigger: 'blur' }
          ],
          mobile: [
    ...
    methods: {
      init (id) {				
		...
		//查询企业名称
		this.$http({
			url:this.$http.adornUrl(`/qualifapply/devcarqualifapply/orgname`),
			method:'post'
		}).then(({data})=>{
			if (data && data.code === 0) {
		...
		this.uploadUrl_szhy = this.$http.adornUrl(`/qualifapply/devcarqualifapply/upload?token=${this.$cookie.get('token')}&applyid=${this.dataForm.id}&attchmenttype=01`)
		this.uploadUrl_sfz = this.$http.adornUrl(`/qualifapply/devcarqualifapply/upload?token=${this.$cookie.get('token')}&applyid=${this.dataForm.id}&attchmenttype=02`)	
      },
	  // 上传之前
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
		  ...
		  else if(!(this.fileListSzhy.length==1&&this.fileListSfz.length==2)){
			this.$message.error('请按照要求上传相关图片资料！')
		  }else{
			if (valid) {
				this.$http({
				  url: this.$http.adornUrl(`/qualifapply/devcarqualifapply/save`),
				  method: 'post',
