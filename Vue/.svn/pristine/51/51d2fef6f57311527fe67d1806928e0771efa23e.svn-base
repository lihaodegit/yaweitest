  ...
  export default {
    ...
    methods: {
		...
		prelCount(){
			//查询-初审待提交数量
			this.$http({
				url:this.$http.adornUrl('/technologyDb/unisubmit/prelcount'),
				method:'post'				
			}).then(({data})=>{
				if(data&&data.code===0){
		...
		retrialCount(){
			//查询复审待提交
			this.$http({
				url:this.$http.adornUrl('/technologyDb/unisubmit/retrialcount'),
				method:'post'
			}).then(({data})=>{
				if(data&&data.code===0){
		...
		prelUniSubmit(){
			this.$http({
				url:this.$http.adornUrl('/technologyDb/unisubmit/prelsubmit'),
				method:'post'
			}).then(({data})=>{
				if(data&&data.code===0){		
					this.$message({
		...
		retrialUniSubmit(){
			this.$http({
				url:this.$http.adornUrl('/technologyDb/unisubmit/retrialsubmit'),
				method:'post'
			}).then(({data})=>{
				if(data&&data.code===0){
					this.$message({
		...
		applyList(flag){
			...
			if(flag=='1'){				
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'kjj',flag:'1'}});
			}
			//初审已提交列表
			else if(flag=='2'){
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'kjj',flag:'2'}});
			}
			//复审待提交列表
			else if(flag=='3'){
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'kjj',flag:'3'}});
			}
			//复审已提交列表
			else if(flag=='4'){
				...
				//跳转列表
				this.$router.push({path:'/unisubmit-applylist',name:"unisubmit-applylist",params: {type: 'kjj',flag:'4'}});
			}
		}
    }
  }
